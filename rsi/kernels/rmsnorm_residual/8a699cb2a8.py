...
@triton.autotune(
    configs=[
        ...
        # ROWS_PER_CTA=2: share weight load across 2 rows
        ...
        # ROWS_PER_CTA=4: share weight load across 4 rows (good weight amortization for prefill)
        ...
        # ROWS_PER_CTA=8: further amortize weight load across 8 rows (better for prefill T=4096)
        # weight bytes / total bytes: 4KB / (8*3*4KB) = 4.2% vs 7.7% for ROWS_PER_CTA=4
        triton.Config({'BLOCK_SIZE': 2048, 'ROWS_PER_CTA': 8}, num_warps=4,  num_stages=2),
        triton.Config({'BLOCK_SIZE': 2048, 'ROWS_PER_CTA': 8}, num_warps=8,  num_stages=2),
        triton.Config({'BLOCK_SIZE': 2048, 'ROWS_PER_CTA': 8}, num_warps=16, num_stages=2),
        triton.Config({'BLOCK_SIZE': 2048, 'ROWS_PER_CTA': 8}, num_warps=8,  num_stages=3),
        triton.Config({'BLOCK_SIZE': 2048, 'ROWS_PER_CTA': 8}, num_warps=16, num_stages=3),
    ],
    key=['T'],
)
@triton.jit
def rmsnorm_residual_kernel(
    ...
    for r in tl.static_range(ROWS_PER_CTA):
        row = row_start + r
        # evict_first: streaming inputs that won't be reused
